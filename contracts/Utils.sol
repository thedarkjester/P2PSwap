...
library Utils {
  ...
  /**
   * @notice Gas efficient swap hashing using inline assembly.
   * @dev There are 8 items in the struct, each using 32 bytes in memory when used,
   * so to hash it we use 0x100 (256), or 8*32 (256) bytes.
   * @param _swap The full Swap struct.
   */
  function hashErc20Swap(IERC20Swapper.Swap memory _swap) internal pure returns (bytes32 swapHash) {
    assembly {
      let mPtr := mload(0x40)
      mcopy(mPtr, _swap, 0x100)
      swapHash := keccak256(mPtr, 0x100)
